...
class XDistralLearner:
    ...
    def optimize_pi_0(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str):
        ...
        for t in range(batch.max_seq_length):
            ...
            pi_0_target_agent_outs = self.target_mac.forward(batch, t=t, task=task)
            pi_0_mac_out.append(pi_0_agent_outs)
            pi_0_target_mac_out.append(pi_0_target_agent_outs)

        pi_0_mac_out = th.stack(pi_0_mac_out, dim=1)  # Concat over time
         # We don't need the first timesteps Q-Value estimate for calculating targets
        pi_0_target_mac_out = th.stack(pi_0_target_mac_out[1:], dim=1)  # Concat across time

        # Pick the Q-Values for the actions taken by each agent
        pi_0_chosen_action_qvals = th.gather(pi_0_mac_out[:, :-1], dim=3, index=actions).squeeze(3)  # Remove the last dim

        # Mask out unavailable actions
        pi_0_target_mac_out[avail_actions[:, 1:] == 0] = -9999999

        # Max over target Q-Values
        if self.main_args.double_q:
            # Get actions that maximise live Q (for double q-learning)

            pi_0_mac_out_detach = pi_0_mac_out.clone().detach()
            pi_0_mac_out_detach[avail_actions == 0] = -9999999
            cur_max_actions = pi_0_mac_out_detach[:, 1:].max(dim=3, keepdim=True)[1]
            pi_0_target_max_qvals = th.gather(pi_0_target_mac_out, 3, cur_max_actions).squeeze(3)

        else:
            pi_0_target_max_qvals = pi_0_target_mac_out.max(dim=3)[0]

        # Mix
        bs, seq_len = pi_0_chosen_action_qvals.size(0), pi_0_chosen_action_qvals.size(1)
        task_repre = self.mac.get_task_repres(task, require_grad=False)[None, None, ...].repeat(bs, seq_len, 1, 1)
        if self.mixer is not None:
            pi_0_chosen_action_qvals = self.mixer(pi_0_chosen_action_qvals, batch["state"][:, :-1], task_repre, self.task2decomposer[task])
            pi_0_target_max_qvals = self.target_mixer(pi_0_target_max_qvals, batch["state"][:, 1:], task_repre, self.task2decomposer[task])


        # Calculate 1-step Q-Learning targets
        pi_0_targets = rewards + self.main_args.gamma * (1 - terminated) * pi_0_target_max_qvals

        # Td-error
        td_error = (pi_0_chosen_action_qvals - pi_0_targets.detach())
        ...
        # for t in range(batch.max_seq_length):
        #     # 这里注意是forward_pi_i
        #     '''特意改成forward试试，上次跑出的效果很好的好像就是这样'''
        #     pi_i_agent_outs = self.mac.forward(batch, t=t, task=task)
        #     pi_i_mac_out.append(pi_i_agent_outs)
        # pi_i_mac_out = th.stack(pi_i_mac_out[:-1], dim=1)  # Concat over time

        
        # # 玻尔兹曼最优策略
        # pi_i_chose_maxq_action = self.bolz_policy(pi_0_mac_out_detach, pi_i_mac_out) #tensor(32, 69, 10)
        # pi_i_chose_maxq_action = pi_i_mac_out.max(dim=3)[1] #tensor(32, 69, 10)

        # # # 当前选择的动作联合动作空间, 计算pi_0与pi_i的KL散度的距离会用到
        # # 动作数和智能体数目
        # action_num = pi_i_mac_out.shape[-1] #18
        # agent_num = pi_i_mac_out.shape[2] # 10
        ...
        #         if t==0:
        #             if agent==0:
        #                 a = copy.deepcopy(pi_i_chose_maxq_action) 
        ...
        # # # 计算近似联合动作空间的total_action_q
        #  # 遍历近似动作空间中的每一个动作计算pi_0
        # pi_0_total_action_q_list = []
        # pi_i_total_action_q_list = []

        # for action in action_space:
        #     action = action.unsqueeze(-1)
        #     pi_0 = pi_0_mac_out[:, :-1].gather(dim=-1, index=action) # tensor(32, 69, 10, 1)
        #     pi_i = pi_i_mac_out.gather(dim=-1, index=action)
        ...
        #         pi_i = self.pi_mixers[task](pi_i, batch["state"][:, :-1])
                
        #     pi_0_total_action_q_list.append(pi_0)
        #     pi_i_total_action_q_list.append(pi_i)

        # pi_0_total_action_q_list = th.stack(tensors=pi_0_total_action_q_list, dim=2).squeeze(-1) # tensor(batch_size, seq_len, pi_0_num)    
        # pi_i_total_action_q_list = th.stack(tensors=pi_i_total_action_q_list, dim=2).squeeze(-1)

        # # 先试试jf散度
        # kl_loss = self.kl_divergence_loss(pi_i_total_action_q_list.detach(), pi_0_total_action_q_list, 1).unsqueeze(-1)
        # mask = mask.expand_as(kl_loss)
        # masked_kl_loss = (mask*kl_loss).sum()/mask.sum()
        # loss += masked_kl_loss
        ...
        if t_env - self.task2train_info[task]["log_stats_t"] >= self.task2args[task].learner_log_interval:
            ...
            self.logger.log_stat(f"{task}/q_taken_mean", (pi_0_chosen_action_qvals * mask).sum().item() / (
                        mask_elems * self.task2args[task].n_agents), t_env)
            self.logger.log_stat(f"{task}/target_mean",
                                 ( pi_0_targets* mask).sum().item() / (mask_elems * self.task2args[task].n_agents), t_env)
            self.task2train_info[task]["log_stats_t"] = t_env
            
    # 最小化pi_i和pi_0之间的距离        
    def minmize_policy_dist(self, batchs, t_env: int, episode_num: int):
        loss = 0
        for task in self.task2args:
            batch = batchs[task]
            batch.to(self.task2args[task].device)
            ...
            for t in range(batch.max_seq_length):
                pi_0_agent_outs = self.mac.forward(batch, t=t, task=task)
                '''修改：cyd'''
                pi_i_agent_outs = self.mac.forward_pi_i(batch, t=t, task=task)
                pi_0_mac_out.append(pi_0_agent_outs)
                pi_i_mac_out.append(pi_i_agent_outs)

            # 在第二维连接，第二维是序列长度
            pi_0_mac_out = th.stack(pi_0_mac_out, dim=1)  # Concat over time
            pi_i_mac_out = th.stack(pi_i_mac_out[:-1], dim=1)
            
            # 最优玻尔兹曼策略
            pi_i_mac_out = self.bolz_policy(pi_0_mac_out, pi_i_mac_out)
            pi_i_chose_maxq_action = pi_i_mac_out.max(dim=3)[1] #tensor(32, 69, 10)

            # '''从agent维度最下化pi_0到pi_i的距离'''
            # pi_i_chose_max_qvals = pi_i_mac_out.max(dim=3)[0]
            # pi_0_chose_max_qvals = th.gather(pi_0_mac_out[:,:-1], 3, pi_i_chose_maxq_action.unsqueeze(-1)).squeeze(-1)
            # agnet_kl_ls = self.kl_divergence_loss(pi_i_chose_max_qvals.detach(), pi_0_chose_max_qvals,1).unsqueeze(-1)
            # mask = mask.expand_as(agnet_kl_ls)
            # agnet_kl_ls = (mask*agnet_kl_ls).sum()/mask.sum()

            # loss += agnet_kl_ls


            '''从Qtot维度最下化pi_0到pi_i的距离'''
            # # 当前选择的动作联合动作空间, 计算pi_0与pi_i的KL散度的距离会用到
            # 动作数和智能体数目
            action_num = pi_i_mac_out.shape[-1] #18
            agent_num = pi_i_mac_out.shape[2] # 10
    ...
    def td_optimize_pi_i(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str):
        # Get the relevant quantities
        rewards = batch["reward"][:, :-1]
        actions = batch["actions"][:, :-1]
        terminated = batch["terminated"][:, :-1].float()
        ...
        for t in range(batch.max_seq_length):
            pi_i_agent_outs = self.mac.forward_pi_i(batch, t=t, task=task)
            pi_i_target_agent_outs = self.target_mac.forward_pi_i(batch, t=t, task=task)
            pi_i_mac_out.append(pi_i_agent_outs)
            pi_i_target_mac_out.append(pi_i_target_agent_outs)

        pi_i_mac_out = th.stack(pi_i_mac_out, dim=1)  # Concat over time
         # We don't need the first timesteps Q-Value estimate for calculating targets
        pi_i_target_mac_out = th.stack(pi_i_target_mac_out[1:], dim=1)  # Concat across time

        # Pick the Q-Values for the actions taken by each agent
        pi_i_chosen_action_qvals = th.gather(pi_i_mac_out[:, :-1], dim=3, index=actions).squeeze(3)  # Remove the last dim

        # Mask out unavailable actions
        pi_i_target_mac_out[avail_actions[:, 1:] == 0] = -9999999

        # Max over target Q-Values
        if self.main_args.double_q:
            # Get actions that maximise live Q (for double q-learning)

            pi_i_mac_out_detach = pi_i_mac_out.clone().detach()
            pi_i_mac_out_detach[avail_actions == 0] = -9999999
            cur_max_actions = pi_i_mac_out_detach[:, 1:].max(dim=3, keepdim=True)[1]
            pi_i_target_max_qvals = th.gather(pi_i_target_mac_out, 3, cur_max_actions).squeeze(3)

        else:
            pi_i_target_max_qvals = pi_i_target_mac_out.max(dim=3)[0]

        # Mix
        bs, seq_len = pi_i_chosen_action_qvals.size(0), pi_i_chosen_action_qvals.size(1)
        task_repre = self.mac.get_task_repres(task, require_grad=False)[None, None, ...].repeat(bs, seq_len, 1, 1)
        if self.pi_mixers[task] is not None:
            pi_i_chosen_action_qvals = self.pi_mixers[task](pi_i_chosen_action_qvals, batch["state"][:, :-1])
            pi_i_target_max_qvals = self.target_pi_mixers[task](pi_i_target_max_qvals, batch["state"][:, 1:])


        # Calculate 1-step Q-Learning targets
        pi_i_targets = rewards + self.main_args.gamma * (1 - terminated) * pi_i_target_max_qvals

        # Td-error
        td_error = (pi_i_chosen_action_qvals - pi_i_targets.detach())
        mask = mask.expand_as(td_error)
        # 0-out the targets that came from padded data
        masked_td_error = td_error * mask
        # Normal L2 loss, take mean over actual data
        ...
        re_loss = self.cal_res_loss(batch, pi_i_chosen_action_qvals)
