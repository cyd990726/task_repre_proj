...
class SotaXAgent(nn.Module):
    ...
    def __init__(self, task2input_shape_info, task2decomposer, task2n_agents, decomposer, args):
        ...
        wrapped_obs_own_dim = obs_own_dim + args.id_length + n_actions_no_attack + 1   #id:4
    ...
    def forward(self, inputs, hidden_state, task_repre, task):
        ...
        # compute wo_action_q
        wo_action_input = th.cat([h, task_repre], dim=-1)
        wo_action_q = self.wo_action_layer(wo_action_input)
        
        # compute action_q
        enemy_hidden = self.enemy_embed(enemy_feats)
        attack_action_input = th.cat([enemy_hidden, h.unsqueeze(0).repeat(enemy_feats.size(0), 1, 1), task_repre.unsqueeze(0).repeat(enemy_feats.size(0), 1, 1)], dim=-1)
