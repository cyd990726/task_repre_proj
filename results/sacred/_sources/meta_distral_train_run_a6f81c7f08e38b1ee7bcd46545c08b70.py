        ...
        for task in train_tasks: 
            ...
            if task2buffer[task].can_sample(batch_size_train):
                ...
                for _run in range(batch_size_run):
                    ...
                    # 进行dynamic_train
                    if task2pretrain_phase[task]:
                        terminated = learner.dynamic_train(episode_sample, task2runner[task].t_env, episode, task)
                    else: 
                        # 训练pi_i
                    #     learner.optimize_pi_i(episode_sample, task2runner[task].t_env, episode, task)
                    #    # learner.td_optimize_pi_i(episode_sample, task2runner[task].t_env, episode, task)
                    #     learner.optimize_pi_0(episode_sample, task2runner[task].t_env, episode, task)
                        learner.rl_train(episode_sample, task2runner[task].t_env, episode, task)
                        # if(flag == len(train_tasks)-1):
                        #     # 最小化pi_0和pi_i之间的距离
                        #     # 采样三次
                        #     batchs = {}
                        #     for task_name in train_tasks:
                        #         batch = task2buffer[task_name].sample(batch_size_train)
                                
                        #         max_ep_t = batch.max_t_filled()
                        #         batch = batch[:, :max_ep_t]
                            
                        #         if batch.device != task2args[task_name].device:
                        #             batch.to(task2args[task_name].device)
                        #         batchs[task_name] = batch

                        #     learner.minmize_policy_dist(batchs, task2runner[task].t_env, episode)      
                    if terminated:
                        break    
                if terminated:
                    tot_run_time[task] += (time.time()-task_start_time[task])
                    # 检查当前task是否仅仅进行表征学习(only_repre_learning),如果是则直接标记task已结束。
            ...
            if not task2pretrain_phase[task]:
                ...
                if (task2runner[task].t_env - task2train_info[task]["last_test_T"]) / task2args[task].test_interval >= 1.0:
                    # 打印一下各个任务的耗时
                    # for task_name in train_tasks:
                    logger.console_logger.info("[任务 {}] 耗时: {}".format(task, time_str(tot_run_time[task])))
                    "修改开始：ljl"
                    logger.console_logger.info("[任务 {}] 胜率为: {}".format(task, logger.stats[task]))
                    "修改结束:ljl"
                ...
                # 因为有batch_size_run个进程，每个进程收集一个episode的数据。所以要加batch_size_run
